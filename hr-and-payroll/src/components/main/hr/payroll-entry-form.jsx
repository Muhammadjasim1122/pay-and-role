...
export default function PayrollEntryForm({ isSidebarOpen = true, editId = null }) {
  const { addPayrollEntry, updatePayrollEntry, getPayrollEntryById } = usePayroll();
  ...
  useEffect(() => {
    if (editId) {
      const entry = getPayrollEntryById(editId);
      if (entry) {
        setFormData({
          ...
          salaryStructure: entry.salaryStructure || '',
          base: entry.base?.toString() || '',
          grossPay: entry.grossPay?.toString() || '',
          totalDeduction: entry.totalDeduction?.toString() || '',
          netPay: entry.netPay?.toString() || '',
          status: entry.status || 'Draft'
        });
      }
    }
  }, [editId, getPayrollEntryById]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await updatePayrollEntry(editId, payload);
        setToastType('success');
        setToastMessage('Payroll entry updated successfully!');
      } else {
        await addPayrollEntry(payload);
        setToastType('success');
        setToastMessage('Payroll entry saved successfully!');
      }
      setShowToast(true);
  
      setTimeout(() => {
        const event = new CustomEvent('setActiveContent', { detail: 'payroll-entry' });
        window.dispatchEvent(event);
        window.history.pushState({ activeContent: 'payroll-entry' }, '', '/');
      }, 500);
    } catch (error) {
      console.error('Failed to save payroll entry:', error);
      setToastType('error');
      setToastMessage(error.message || 'Failed to save payroll entry. Please try again.');
      setShowToast(true);
    }
  };
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-semibold text-gray-900">
              {editId ? 'Edit Payroll Entry' : 'New Payroll Entry'}
            </h1>
